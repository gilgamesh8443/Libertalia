...
> [!TIP]
> Many common video and audio codecs are not supported on certain browsers (hevc, eac3, etc.), using [FFmpeg](https://www.ffmpeg.org/) you can convert videos to codecs more commonly compatible with browsers (av1, h264, aac, etc.). Unless transcoded ahead of time, the server will attempt to transcode video in realtime. For processors without hardware accelerated encoders, this can be noticeably slow.
